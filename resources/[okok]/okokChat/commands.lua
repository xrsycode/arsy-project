...
if Config.EnablePoliceCommand then
	RegisterCommand(Config.PoliceCommand, function(source, args, rawCommand)
		...
		if job == Config.PoliceJobName then
			TriggerClientEvent('chat:addMessage', -1, {
				template = '<div class="chat-message police"> <b><span style="color: #4a6cfd">🚔{0}</span>&nbsp;<span style="color: #ffffff">| KEPOLISIAN NEW LIFE</span></span>&nbsp;<span style="font-size: 14px; color: #e1e1e1;">{2}</span></b><div style="margin-top: 5px;">{1}</div></div>',
				args = { playerName, message, time }
			})
		end
	end)
...
if Config.EnableAmbulanceCommand then
	RegisterCommand(Config.AmbulanceCommand, function(source, args, rawCommand)
		...
		if job == Config.AmbulanceJobName then
			TriggerClientEvent('chat:addMessage', -1, {
				template = '<div class="chat-message ambulance"> <b><span style="color: #ff0033">🚑{0}</span>&nbsp;<span style="color: #ffffff">| EMS NEW LIFE</span></span>&nbsp;<span style="font-size: 14px; color: #e1e1e1;">{2}</span></b><div style="margin-top: 5px; font-weight: 300;">{1}</div></div>',
				args = { playerName, message, time }
			})
		end
	end)
...
if Config.EnableMechanicCommand then
	RegisterCommand(Config.MechanicCommand, function(source, args, rawCommand)
		...
		if job == Config.MechanicJobName then
			TriggerClientEvent('chat:addMessage', -1, {
				template = '<div class="chat-message mechanic"> <b><span style="color: #eaab00">🚘{0}</span>&nbsp;<span style="color: #ffffff">| MECHANIC NEW LIFE</span></span>&nbsp;<span style="font-size: 14px; color: #e1e1e1;">{2}</span></b><div style="margin-top: 5px; font-weight: 300;">{1}</div></div>',
				args = { playerName, message, time }
			})
		end
	end)
...
if Config.EnableUwuCommand then
	RegisterCommand(Config.UwuCommand, function(source, args, rawCommand)
		local xPlayer = QBCore.Functions.GetPlayer(source)
		local length = string.len(Config.UwuCommand)
		local message = rawCommand:sub(length + 1)
		local time = os.date(Config.DateFormat)
		local playerName = Config.ShowIDOnMessageForEveryone and xPlayer.PlayerData.charinfo.firstname.." "..xPlayer.PlayerData.charinfo.lastname.." ["..source.."]" or xPlayer.PlayerData.charinfo.firstname.." "..xPlayer.PlayerData.charinfo.lastname
		local job = xPlayer.PlayerData.job.name

		if job == Config.UwuJobName then
			TriggerClientEvent('chat:addMessage', -1, {
				template = '<div class="chat-message uwu"> <b><span style="color: #e966a0">🐱{0}</span>&nbsp;<span style="color: #ffffff">| PEDAGANG NEW LIFE</span></span>&nbsp;<span style="font-size: 14px; color: #e1e1e1;">{2}</span></b><div style="margin-top: 5px; font-weight: 300;">{1}</div></div>',
				args = { playerName, message, time }
			})
		end
	end)
